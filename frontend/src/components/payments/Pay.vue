				...
				>
					...
					>
						...
						<v-data-table
							...
							:items="paginatedUnallocatedPayments"
							item-key="name"
							class="elevation-1 mt-0"
						...
						>
							...
							<template v-slot:bottom>
								<div class="text-center compact-pagination">
									<span class="text-caption page-info">
										{{
											paginatedUnallocatedPayments.length
												? (paymentsPage - 1) * paymentsItemsPerPage + 1
												: 0
										}}
										-
										{{
											Math.min(
												paymentsPage * paymentsItemsPerPage,
												unallocated_payments.length
											)
										}}
						...
						<div class="section-header mb-4">
							<v-card class="modern-section-card" variant="flat">
								<v-card-title class="section-title">
									...
									<div class="section-text">
										<div class="section-main-title">
											{{ __("Search Mpesa Payments") }}
										</div>
									</div>
									<div
										v-if="total_selected_mpesa_payments"
										class="section-badge"
									>
										<v-chip color="primary" variant="flat" size="small">
											{{ __("Selected") }}:
						...
						>
							...
							<template v-slot:bottom>
								<div class="text-center compact-pagination">
									<span class="text-caption page-info">
										{{
											paginatedMpesaPayments.length
												? (mpesaPage - 1) * mpesaItemsPerPage + 1
												: 0
										}}
										-
										{{
											Math.min(
												mpesaPage * mpesaItemsPerPage,
												mpesa_payments.length
											)
										}}
							...
							<div class="subsection-header mb-4">
								<v-card class="modern-subsection-card" variant="flat">
									<v-card-title class="subsection-title">
										<v-icon class="subsection-icon" size="20"
	...
	methods: {
		async check_opening_entry() {
			...
			if (r) {
				...
				vm.eventBus.emit("payments_register_pos_profile", r);
				vm.eventBus.emit("set_company", r.company);
				this.set_payment_methods();
				this.pos_profile_search = r.pos_profile.name;
		...
		async view(id) {
			var vm = this;
			const r = await call("pospire.pospire.api.posapp.search_invoices_with_items", {
				invoice_name: id,
				company: vm.company,
			});
			if (r && r.length > 0 && r[0].items) {
				vm.eventBus.emit("open_paid", r[0].items);
			} else {
		...
		async get_available_pos_profiles() {
			...
			const r = await call("pospire.pospire.api.payment_entry.get_available_pos_profiles", {
				company: this.company,
				currency: this.pos_profile.currency,
			});
			if (r) {
				this.pos_profiles_list = r;
			}
		},
		...
		async get_outstanding_invoices() {
			this.invoices_loading = true;
			const r = await call("pospire.pospire.api.payment_entry.get_outstanding_invoices", {
			...
			if (r) {
				this.outstanding_invoices = r;
				this.invoices_loading = false;
			}
		},
		async get_unallocated_payments() {
			...
			const r = await call("pospire.pospire.api.payment_entry.get_unallocated_payments", {
				customer: this.customer_name,
				company: this.company,
				currency: this.pos_profile.currency,
			});
			if (r) {
				this.unallocated_payments = r;
				this.unallocated_payments_loading = false;
			}
		},
		...
		async get_draft_mpesa_payments_register() {
			if (!this.pos_profile.posa_allow_mpesa_reconcile_payments) return;
			const vm = this;
			this.mpesa_payments_loading = true;
			const r = await call("pospire.pospire.api.m_pesa.get_mpesa_draft_payments", {
				company: vm.company,
...
.customer-section-wrapper {
	/* Extra top padding so the outlined field's floating label has room */
	padding-top: 0.5rem;
}

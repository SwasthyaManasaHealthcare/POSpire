					...
					>
						...
						>
							...
							<v-date-picker
								...
								scrollable
								color="primary"
								:min="datetime.add_days(datetime.now_date(), -7)"
								:max="datetime.add_days(datetime.now_date(), 7)"
...
export default {
	...
	methods: {
		...
		updateSalesTeam() {
			if (!this.invoice_doc || typeof this.invoice_doc !== "object") return;
		...
		async update_invoice_from_order(doc) {
			var vm = this;
			const r = await call("pospire.pospire.api.posapp.update_invoice_from_order", {
				data: doc,
			});
			if (r) {
				vm.invoice_doc = r;
		...
		async get_draft_orders() {
			var vm = this;
			const r = await call("pospire.pospire.api.posapp.search_orders", {
				company: this.pos_profile.company,
				currency: this.pos_profile.currency,
			});
			if (r) {
				vm.eventBus.emit("open_orders", r);
			}
		},
		...
		update_item_detail(item) {
			...
			var vm = this;
			call("pospire.pospire.api.posapp.get_item_detail", {
		...
		async set_delivery_charges() {
			...
			this.delivery_charges_rate = 0;
			this.selected_delivery_charge = "";
			const r = await call("pospire.pospire.api.posapp.get_applicable_delivery_charges", {
				company: this.pos_profile.company,
				pos_profile: this.pos_profile.name,
				customer: this.customer,
			});

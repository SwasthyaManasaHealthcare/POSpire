...
def _filter_items(items):
	...
	return [item for item in items if item.get("label") not in CORE_POS_LINKS]
...
def _filter_workspace_response(response):
	...
	if shortcuts:
		shortcuts["items"] = _filter_items(shortcuts.get("items", []))

	quick_lists = response.get("quick_lists")

	if quick_lists:
		quick_lists["items"] = _filter_items(quick_lists.get("items", []))
